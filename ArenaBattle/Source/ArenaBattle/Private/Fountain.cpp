...
AFountain::AFountain()
{
	...
	Water->SetRelativeLocation(FVector(0.0f, 0.0f, 135.f));
	Light->SetRelativeLocation(FVector(0.0f, 0.0f, 195.0f));
	Splash->SetRelativeLocation(FVector(0.0f, 0.0f, 195.0f));


	// Initial SM is Static Mesh
	// Initial PS is Particle System
...
void AFountain::BeginPlay()
{
	...
	ABLOG_S(Warning);
	ABLOG(Warning, TEXT("Actor Name : %s, ID : %d, Location (X : %.3f Y : %.3f Z : %.3f)"), *GetName(), GetActorLocation().X, GetActorLocation().Y, GetActorLocation().Z);
}
